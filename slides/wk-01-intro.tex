...
\begin{itemize}
  ...
  \item Tuesdays 16:45, Room 3.51, tř.\ Svobody 26
    \begin{itemize}
    \item Francis: 90 minutes, \textbf{16:45--18:15}
    \item Pavel: three hours, 16:45--19:45, with the new material in the first
      90 minutes and practice after, so you can finish at home if you must leave
    \end{itemize}
...
\begin{frame}{Write Three Paragraphs (by Sunday \deadlineTask)}
\begin{itemize}
  \item Read all three, then write roughly 400--500 words:
    ...
    \end{itemize}
  \item Post it on Moodle by Sunday \deadlineTask, and bring it to class on \dateSources
  ...
  \item Not graded.  We use it to see how you write; you use it to practise
    reading like a writer.  We come back to it on \dateSources.
\end{itemize}
\end{frame}
...
\toprule
\textbf{Date} & \textbf{Who} & \textbf{What} \\
\midrule
\dateWikiShort      & Pavel   & Wikipedia rules: accounts, sandboxes, the five pillars \\
\dateMessageShort   & Francis & Academic vs.\ encyclopedic writing: what is your message? \\
\dateSourcesShort   & Francis & Finding, judging and citing sources \\
\addlinespace
\breakReadingShort  & ---     & \emph{\breakTitleReading}: no class \\
\addlinespace
\dateFairShort      & Francis & FAIR: making data findable and reusable \\
\dateGoodShort      & Pavel   & Writing a good Wikipedia article \\
\dateCareShort      & Francis & CARE, ethics, and honest persuasion \\
\addlinespace
\breakHolidayShort  & ---     & \emph{Public holiday}: no class \\
\addlinespace
\dateFeedbackShort  & Pavel   & Feedback on Wikipedia articles \\
\dateReviewShort    & Francis & Peer review: how it works \\
\dateWorkshopShort  & Francis & Peer review: doing it --- bring your draft \\
\dateConclusionShort & both   & Conclusions and reflection \\
\bottomrule
\end{tabular}
\vspace{1ex}
...
\begin{itemize}
  ...
  \item \textbf{\dateWiki}: Pavel's session on Wikipedia, in Czech
  \item \textbf{\deadlineTask}: post your three paragraphs on Moodle
...
\begin{frame}{Choosing and Using Evidence (\dateSources)}
  \begin{itemize}
...
\begin{frame}{Sustainable Knowledge (\dateFair)}
...
\begin{frame}{Writing is Revising (\dateWorkshop)}
\begin{itemize}
