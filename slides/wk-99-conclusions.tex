...
% ---- Title metadata ----
\title[Open Knowledge --- Conclusions]{Open Knowledge for a Sustainable Future:\\ Research, Ethics, and Wikipedia}
...
% NOTE (2026-09-20): APA 7 wants a page number with every direct quotation.
% The book quotations below (King, Le Guin, Butler, Murakami, Coates, Goodall,
% Capek) are verbatim and correctly attributed, but none carries a page yet.
% Add \citep[p.~NN]{key} from your own copies before teaching.
\subsection{Writers}
\label{sec:writers}
...
\item What is his advice?
  \begin{itemize}
    \item \textbf{Read a lot, write a lot.}\\
      ``If you want to be a writer, you must do two things above all others: read a lot and write a lot.  There's no way around these two things that I'm aware of, no shortcut.'' \citep{king:onwriting}
      \begin{itemize}
...
\item What is her advice?
  \begin{itemize} 
    \item \textbf{Practice the craft deliberately.}\\
      In \emph{Steering the Craft}, Le Guin treats writing as a set of skills (rhythm, syntax, point of view) that can be practised in focused exercises, not just ``inspired'' \citep{leguin:steering}.
    \item To try:
      \begin{itemize}
...
{\footnotesize \textbf{Even here:} that Socrates line is in no surviving
Platonic text.  The nearest is \emph{Phaedo} 115e, on false words.  A careful
writer on the ethics of language, passing on a quotation she had not checked.}
...
\item What is her advice?
  \begin{itemize}
    \item \textbf{Habit beats inspiration.}\\
      ``First forget inspiration. Habit is more dependable. Habit will sustain you whether you're inspired or not.''\footnote{\cite{butler:furor-scribendi}}
      \begin{itemize}
...
\item What is his advice?
  \begin{itemize}
  \item \textit{I think the first task for the aspiring novelist is to read
      tons of novels.  Sorry to start with such a commonplace observation, but
      no training is more crucial.} \citep{Murakami:2022}
  ...
  \item Who is he?
    ...
    \item Explored libraries, labyrinths, and infinite texts.
     \begin{quote}
      ...
      \hfill \citep{Borges:SevenNights}
    \end{quote}
    From his lecture ``Blindness'': he was made director of Argentina's
    National Library in 1955, as his sight was failing.  He says the same
    thing in verse in ``Poema de los dones'' (1960).
  \end{itemize}  
  \item What is his advice?
    \begin{quote}
...
\begin{frame}{Chinua Achebe}
 ...
 \end{quote}
 \hfill {\footnotesize Achebe is quoting a proverb, not coining it \citep{Achebe:1994}}
\vspace{2ex}
 \begin{quote}
...
\begin{frame}{Shonda Rhimes — Advice}
  ...
  \item Write even when you don't feel like it
    \begin{quote}
    ``Dreams do not come true just because you dream them.  It's hard work
    that makes things happen.'' \hfill \citep{rhimes:dartmouth2014}
  \end{quote}
   \begin{itemize}
...
\begin{frame}{Ta-Nehisi Coates — thinking through writing}
  ...
  \item What is his advice?
    \begin{quote}
      ...
      version of what my mother had taught me all those years ago --- the
      craft of writing as the art of thinking.
      \hfill \textnormal{\citep{coates:between}}
    \end{quote}
    \begin{itemize}
...
\begin{frame}{Ta-Nehisi Coates --- economy of truth}
  \begin{quote}
    Poetry aims for an economy of truth --- loose and useless words must be
    discarded, and I found that these loose and useless words were not
    separate from loose and useless thoughts.
    \hfill \textnormal{\citep{coates:between}}
  \end{quote}
  \begin{itemize}
  \item Vague language hides weak thinking
  \item Precision is a moral as well as an intellectual task
...
\begin{frame}{Jane Goodall}
  \begin{itemize}
    ...
    \item What is her advice?
      \begin{itemize}
        \item \textit{Only if we understand, can we care.  Only if we care, we will help.  Only if we help, we shall be saved.}
\\ \hfill \citep{goodall:40years}
  ...
  \\ \hfill   \citep{goodall:shadowsofman,goodall:chimpanzees}
        \item \textit{What you do makes a difference, and you have to decide what kind of difference you want to make.}
          \\ \hfill {\footnotesize widely reported; we could not trace it to a published text}
      \end{itemize}
  \end{itemize}
\end{frame}
    ...
    \item What is her advice?
      \begin{itemize}
        \item Replace fear with understanding.\\
          ...
          \begin{itemize}
          \item \textbf{But watch this quotation grow.}  Nobody has traced
            the sentence to anything Curie wrote; the earliest print trace is
            a French medical article of 1951.  The second sentence usually
            attached to it --- \emph{now is the time to understand more, so
            that we may fear less} --- is Glenn Seaborg's, written in 1968
            \citep{seaborg1968fear}.
          \item If you cannot find the original, do not quote it.
          \end{itemize}
    ...
    \item What is her advice?
      \begin{itemize}
        \item Ask who pays the price.\\
          Bender, Gebru, McMillan-Major and Shmitchell argue that researchers
          should weigh the environmental and financial costs of training a
          model, curate datasets deliberately, and think through downstream
          harms \citep{bender:stochastic-parrots}.  (Paraphrase, not a quotation.)
    ...
    \item What is his advice?
      \begin{itemize}
        \item Clarity and correctness first.\\
          ...
          \\ \hfill \textit{Notes on Structured Programming} \citep[\S3]{dijkstra:structured}
    ...
    \item What is his advice?
      \begin{itemize}
        ...
        \item Education should be universal and systematic.\\
          ...
          \\ „Všechny všemu všestranně.“ \hfill \citep{comenius:didactica-magna}
          \begin{itemize}
...
\begin{frame}{Putting it together}
  \begin{enumerate}
    ...
    \item Treat clarity as craft:
      \begin{itemize}
        \item Le Guin, Dijkstra, Coates, \v{C}apek: clarity comes through practice and revision
      \end{itemize}
...
\begin{frame}{What you have made}
\begin{itemize}
  \item A \textbf{Wikipedia article} that is public, and that people will
    read and go on editing after this course ends
  \item A \textbf{paper} of your own, with sources you found, read and cited
  \item Reviews you wrote for other people --- and the revisions you made
    because of theirs
  \item The habit underneath all of it: \textbf{check the source, then write}
\end{itemize}
\vspace{2ex}
That is what sustainable knowledge looks like: work that others can find,
trust, and build on.
\end{frame}
...
\small
\begin{itemize}
  \item The writers and scientists quoted here, and the people who kept
    their words in print
  \item \textcite{openai-chatgpt-2025-03-14} was used to format the
    references, and generate a first draft of the slides
  \aicheck
\end{itemize}
\end{frame}
